...
class ProjectionVarianceService:
    ...
    async def calculate_variance(
        ...
        try:
            ...
            if use_historical:
                variance_model = await self._build_historical_variance(player.player_id, projection.season)
            else:
                variance_model = self.variance_coefficients.get(player.position, {})
            ...
            for stat_name in stat_fields:
                ...
                coef = variance_model.get(stat_name, self.variance_coefficients.get(player.position, {}).get(stat_name, 0.3))
                
                # Calculate standard deviation
                std_dev = value * coef
                
                # Adjust for games if needed
                if adjust_for_games and projection.games > 0:
                    # Variance scales with sqrt(n) for independent events
                    std_dev = std_dev / math.sqrt(projection.games) * math.sqrt(17)
                
                # Calculate confidence intervals
                intervals = {}
                for conf_level, z_score in self.confidence_z.items():
                    lower = max(0, value - z_score * std_dev)
    ...
    async def generate_projection_range(
        ...
        try:
            ...
            conf_level = min(self.confidence_z.keys(), key=lambda x: abs(x - confidence))
            ...
            for stat in stat_fields:
                if stat in variance_data:
                    ...
                    interval = variance_data[stat]["intervals"][f"{conf_level:.2f}"]
                    
                    # Low projection
                    projection_range["low"][stat] = interval["lower"]
                    
                    # Median projection (same as base)
                    projection_range["median"][stat] = value
                    
                    # High projection
                    projection_range["high"][stat] = interval["upper"]
            
            # Add fantasy points
            if "half_ppr" in variance_data:
                interval = variance_data["half_ppr"]["intervals"][f"{conf_level:.2f}"]
                projection_range["low"]["half_ppr"] = interval["lower"]
                projection_range["median"]["half_ppr"] = projection.half_ppr
                projection_range["high"]["half_ppr"] = interval["upper"]
    ...
    async def _build_historical_variance(
        self,
        player_id: str,
        season: int
    ) -> Dict[str, float]:
        ...
        try:
            ...
            if not game_stats:
                ...
                return self.variance_coefficients.get(player.position, {})
            ...
            if player.position == "QB":
                ...
                pass_attempts = [float(g.stats.get('att', 0)) for g in game_stats if 'att' in g.stats]
                ...
                if pass_attempts and len(pass_attempts) >= 8:
                    variance_model['pass_attempts'] = np.std(pass_attempts) / max(1, np.mean(pass_attempts))
                if completions and len(completions) >= 8:
                    variance_model['completions'] = np.std(completions) / max(1, np.mean(completions))
                ...
                rec_yards = [float(g.stats.get('rec_yds', 0)) for g in game_stats if 'rec_yds' in g.stats]
    ...
    async def _calculate_fantasy_point_variance(
        ...
        stat_variances: Dict[str, Dict[str, float]],
        position: str
    ) -> Dict[str, Any]:
        """
        ...
        for stat1, weight1 in fp_weights.items():
            ...
            for stat2, weight2 in fp_weights.items():
                ...
                std_dev1 = stat_variances[stat1]["std_dev"]
                std_dev2 = stat_variances[stat2]["std_dev"]
                covariance = corr * std_dev1 * std_dev2
                
                # Add to variance sum (twice for each pair)
                variance_sum += 2 * weight1 * weight2 * covariance
        ...
        intervals = {}
        for conf_level, z_score in self.confidence_z.items():
            lower = max(0, fp_base - z_score * fp_std_dev)
            upper = fp_base + z_score * fp_std_dev
            intervals[f"{conf_level:.2f}"] = {
                "lower": round(lower, 2),
                "upper": round(upper, 2)
            }
        ...
        return {
            ...
            "coef_var": round(fp_std_dev / max(1, fp_base), 3),
            "intervals": intervals
        }
    ...
    async def _create_range_scenarios(
        self,
        projection: Projection,
        projection_range: Dict[str, Dict[str, float]],
        confidence: float
    ) -> Dict[str, str]:
        """
