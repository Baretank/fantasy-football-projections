...
from backend.database.models import Player, Projection, DraftStatus, DraftBoard, RookieProjectionTemplate
...
class DraftService:
    ...
    async def get_draft_board(
        ...
        if order_by == "name":
            ...
            query = query.join(Projection, and_(
        ...
        for status in ["available", "drafted", "watched"]:
            count = self.db.query(Player).filter(
                Player.draft_status == status
            ).count()
        ...
        try:
            ...
            if draft_status == DraftStatus.DRAFTED:
                ...
                if draft_order is None:
                    ...
                    highest = self.db.query(Player).filter(
                        Player.draft_status == DraftStatus.DRAFTED
                    ).order_by(desc(Player.draft_order)).first()
                ...
                if create_projection and player.is_rookie:
                    ...
                    if not existing:
                        ...
                        draft_position = player.draft_pick if player.draft_pick else player.draft_position
                        
                        if draft_position:
                            projection = await self.rookie_projection_service.create_draft_based_projection(
                                player_id=player_id,
                            ...
                            if projection:
                                logger.info(f"Created projection for rookie {player.name} at draft position {draft_position}")
    ...
    async def reset_draft(self) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            count = self.db.query(Player).filter(
                Player.draft_status != DraftStatus.AVAILABLE
            ).count()
    ...
    async def undo_last_draft_pick(self) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            last_pick = self.db.query(Player).filter(
                Player.draft_status == DraftStatus.DRAFTED
            ).order_by(desc(Player.draft_order)).first()
            ...
            if not last_pick:
                return {
                    "success": False,
                    "error": "No drafted players found"
                }
    ...
    async def get_draft_progress(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get count by status
            available_count = self.db.query(Player).filter(
                Player.draft_status == DraftStatus.AVAILABLE
            ).count()
            
            drafted_count = self.db.query(Player).filter(
                Player.draft_status == DraftStatus.DRAFTED
            ).count()
            
            watched_count = self.db.query(Player).filter(
                Player.draft_status == DraftStatus.WATCHED
            ).count()
            ...
            highest_order = self.db.query(Player).filter(
                Player.draft_status == DraftStatus.DRAFTED
            ).order_by(desc(Player.draft_order)).first()
