...
class RookieProjectionService:
    ...
    async def get_rookie_data(self) -> List[Dict]:
        """
        ...
        for rookie_data in rookies:
            try:
                ...
                if not player:
                    ...
                    logger.info(f"Updated team for rookie: {rookie_data['name']} to {rookie_data['team']}")
                ...
                msg = f"Error creating projection for {rookie_data.get('name', 'unknown')}: {str(e)}"
                logger.error(msg)
    ...
    async def _enhance_with_team_context(
        ...
        if player.position == "QB":
            # Adjust pass volume based on team tendencies
            team_pass_att_per_game = team_stats.pass_attempts / 17
            ...
            adjusted_rush_att = (
                model_rush_att_per_game * 0.6 + team_rush_att_per_game * 0.4 * 0.3  # Assume 30% team share
            ) * projection.games * playing_time_pct
            ...
            # Adjust receiving volume
            team_targets_per_game = team_stats.targets / 17
            ...
            # Adjust target volume based on team tendencies
            team_targets_per_game = team_stats.targets / 17
            wr_target_share = 0.7  # Assume WRs get ~70% of targets
            
            adjusted_targets = (
                team_targets_per_game * wr_target_share * 0.2  # Assume rookie gets 20% of WR targets
            ) * projection.games * playing_time_pct
            ...
            adjusted_targets = (
                team_targets_per_game * te_target_share * 0.5  # Assume rookie gets 50% of TE targets if starting
            ) * projection.games * playing_time_pct
