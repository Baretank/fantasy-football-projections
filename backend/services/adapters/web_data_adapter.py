...
class WebDataAdapter:
        ...
        while True:
            try:
                ...
                if e.status == 429:
                    ...
                    logger.warning(f"Rate limited on {url} (retry {retry_count+1}/{self.max_retries})")
    ...
    async def _fetch_html_table(self, url: str, table_id: str = None) -> pd.DataFrame:
        """
    ...
    async def fetch_csv_data(self, url: str) -> pd.DataFrame:
        """
        Fetch CSV data from a URL and convert to DataFrame.
