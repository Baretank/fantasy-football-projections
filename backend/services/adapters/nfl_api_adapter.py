    ...
    async def _make_api_request(
        ...
        while retry_count < self.max_retries:
            try:
                ...
                params_to_use = params if params is not None else {}
                # Ensure session is initialized
                if self.session is None:
                    await self.initialize()
                # session is guaranteed to be non-None after initialize
                assert self.session is not None
                async with self.session.get(url, headers=self.headers, params=params_to_use) as response:
                    if response.status == 200:
                        ...
                        logger.warning(
                            f"Rate limited, retrying in {wait_time}s (attempt {retry_count}/{self.max_retries})"
                        )
                ...
                wait_time = self.retry_delay * (2**retry_count)
                logger.warning(
                    f"Request error, retrying in {wait_time}s (attempt {retry_count}/{self.max_retries}): {str(e)}"
                )
                await asyncio.sleep(wait_time)
