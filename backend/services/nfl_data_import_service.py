from typing import Dict, List, Optional, Any, Tuple, TypedDict, cast
...
# Service-specific TypedDict definitions
class ImportMetricsDict(TypedDict, total=False):
    """Dictionary for tracking import metrics"""
    requests_made: int
    rate_limits_hit: int
    errors: int
...
class GameContextDict(TypedDict, total=False):
    """Dictionary for game context information"""
    opponent: str
    location: str
    result: str
    team_score: int
...
class ImportResultDict(TypedDict, total=False):
    """Dictionary for import operation results"""
    players_added: int
    players_updated: int
    total_processed: int
    weekly_stats_added: int
    teams_processed: int
    totals_created: int
    players_processed: int
    issues_found: int
    issues_fixed: int
    players_validated: int
...
class NFLDataImportService:
    ...
    def end_monitoring(self) -> ImportResultDict:
        """
        End the monitoring session and report metrics.
        
        Returns:
            Dictionary with duration and metrics information
        """
        ...
        # Log detailed stats
        requests_made = safe_float(self.metrics.get("requests_made"), 0)
        if requests_made > 0:
            errors = safe_float(self.metrics.get("errors"), 0)
            error_rate = safe_calculate(errors, requests_made, 0.0) * 100
            self.logger.info(f"Error rate: {error_rate:.2f}%")

        rate_limits = safe_float(self.metrics.get("rate_limits_hit"), 0)
        if rate_limits > 0:
            self.logger.warning(f"Rate limit hit {rate_limits} times")

        # Cast to the expected return type
        return cast(ImportResultDict, {
            "duration_seconds": duration, 
            "metrics": self.metrics
        })

    async def import_season(self, season: int) -> ImportResultDict:
        """
        ...
        try:
            ...
            # Get monitoring results and merge with results
            monitoring_results = self.end_monitoring()
            return cast(ImportResultDict, {**results, **monitoring_results})
            ...
            self._log_import(
                "season_import_error", "error", f"Error importing season {season}: {str(e)}"
            )
    ...
    async def import_players(self, season: int, limit: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for _, row in player_data.iterrows():
                ...
                player_id = None
                # For pandas Series, we need to use row.get() or row[] instead of safe_dict_get
                gsis_id = row.get("gsis_id") if "gsis_id" in row else None
                if gsis_id is not None and not pd.isna(gsis_id):
                    player_id = gsis_id
                else:
                    smart_id = row.get("smart_id") if "smart_id" in row else None
                    if smart_id is not None and not pd.isna(smart_id):
                        player_id = smart_id
                    else:
                        esb_id = row.get("esb_id") if "esb_id" in row else None
                        if esb_id is not None and not pd.isna(esb_id):
                            player_id = esb_id
                        else:
                            # If no ID is available, generate a unique ID
                            display_name = row.get("display_name", "Unknown")
                            self.logger.warning(
                                f"No player ID found for {display_name}. Skipping."
                            )
                            continue
                ...
                # Convert height
                height_inches = None
                height_val = row.get("height") if "height" in row else None
                if (
                    ...
                    try:
                        feet, inches = height_val.split("-")
                        height_inches = int(feet) * 12 + int(inches)
                    except (ValueError, TypeError):
                        # Handle conversion errors safely
                        self.logger.debug(f"Error converting height value: {height_val}")
                        height_inches = None

                # Get team information - team_abbr is the field in the API
                team = row.get("team_abbr", "UNK") if "team_abbr" in row else "UNK"
                if pd.isna(team):
                    team = "UNK"
                ...
                player_dict: PlayerDict = {
                    "player_id": player_id,
                    "name": row.get("display_name", "Unknown") if "display_name" in row else "Unknown",
                    "position": row.get("position", "UNK") if "position" in row else "UNK",
                    "team": team,
                    "status": row.get("status", "Unknown") if "status" in row else "Unknown",
                    "height": height_inches,
                    "weight": row.get("weight") if "weight" in row else None,
                }
                
                # Extra validation - ensure position is not NaN
                if pd.isna(player_dict["position"]):
                    player_dict["position"] = "UNK"
                
                # Ensure name is not NaN
                if pd.isna(player_dict["name"]):
                    player_dict["name"] = "Unknown"
            ...
            self._log_import("player_import", "error", f"Error importing player data: {str(e)}")
            raise Exception(f"Error importing player data: {str(e)}")

    async def import_weekly_stats(
        self, season: int, player_limit: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for _, game in schedules.iterrows():
                if not pd.isna(game.get("game_id")):
                    ...
                    game_info[f"{game['home_team']}_{week}"] = {
                        ...
                        "location": "home",
                        "result": (
                            "W" if game.get("home_score", 0) > game.get("away_score", 0) else "L"
                        ),
                    ...
                    game_info[f"{game['away_team']}_{week}"] = {
                        ...
                        "location": "away",
                        "result": (
                            "W" if game.get("away_score", 0) > game.get("home_score", 0) else "L"
                        ),
            ...
            fantasy_positions = ["QB", "RB", "WR", "TE"]
            query = self.db.query(Player).filter(Player.position.in_(fantasy_positions))

            # If player_limit is specified, add the limit
            if player_limit:
                self.logger.info(
                    f"Limiting weekly stats import to {player_limit} fantasy-relevant players"
                )
                query = query.limit(player_limit)
            else:
                self.logger.info(
                    f"Filtering weekly stats import to fantasy-relevant positions only"
                )

            # Execute query to get players
            players = query.all()
            ...
            self.logger.info(
                f"Filtered weekly stats from {original_count} to {len(weekly_data)} entries for fantasy-relevant players"
            )
            ...
            for _, row in weekly_data.iterrows():
                ...
                team = (
                    row.get("recent_team") if not pd.isna(row.get("recent_team")) else player.team
                )
                ...
                try:
                    ...
                    self.logger.error(
                        f"Error processing game stat for {player.name} week {week}: {str(e)}"
                    )
                    import traceback
            ...
            results = {"weekly_stats_added": stats_added, "errors": errors}
            ...
            self.logger.debug(f"Stack trace: {traceback.format_exc()}")
            self._log_import(
                "weekly_stats_import", "error", f"Error importing weekly stats: {str(e)}"
            )
        ...
        try:
            ...
            for _, row in team_data.iterrows():
                ...
                existing_stats = (
                    ...
                    .filter(TeamStat.team == team_abbr, TeamStat.season == season)
                    .first()
                )

                # The data already contains the calculated fields from our adapter
                # Just prepare team stat data for our model
                team_stat_data: TeamStatsDict = {
                    "team": team_abbr,
                    "season": season,
                    "plays": safe_float(row.get("plays") if "plays" in row else None, 0.0),
                    "pass_percentage": safe_float(row.get("pass_percentage") if "pass_percentage" in row else None, 0.0),
                    "pass_attempts": safe_float(row.get("pass_attempts") if "pass_attempts" in row else None, 0.0),
                    "pass_yards": safe_float(row.get("pass_yards") if "pass_yards" in row else None, 0.0),
                    "pass_td": safe_float(row.get("pass_td") if "pass_td" in row else None, 0.0),
                    "pass_td_rate": safe_float(row.get("pass_td_rate") if "pass_td_rate" in row else None, 0.0),
                    "rush_attempts": safe_float(row.get("rush_attempts") if "rush_attempts" in row else None, 0.0),
                    "rush_yards": safe_float(row.get("rush_yards") if "rush_yards" in row else None, 0.0),
                    "rush_td": safe_float(row.get("rush_td") if "rush_td" in row else None, 0.0),
                    "rush_yards_per_carry": safe_float(row.get("rush_yards_per_carry") if "rush_yards_per_carry" in row else None, 0.0),
                    "targets": safe_float(row.get("targets") if "targets" in row else None, 0.0),
                    "receptions": safe_float(row.get("receptions") if "receptions" in row else None, 0.0),
                    "rec_yards": safe_float(row.get("rec_yards") if "rec_yards" in row else None, 0.0),
                    "rec_td": safe_float(row.get("rec_td") if "rec_td" in row else None, 0.0),
                    "rank": int(safe_float(row.get("rank") if "rank" in row else None, 0.0)),
                }
                ...
                if existing_stats:
                    ...
                    new_team_stat = TeamStat(team_stat_id=str(uuid.uuid4()), **team_stat_data)
            ...
            results = {"teams_processed": teams_processed}
            ...
            self._log_import("team_stats_import", "error", f"Error importing team stats: {str(e)}")
    ...
    async def calculate_season_totals(self, season: int) -> Dict[str, Any]:
        ...
        try:
            ...
            for player in players:
                ...
                game_stats = (
                    self.db.query(GameStats)
                    .filter(GameStats.player_id == player.player_id, GameStats.season == season)
                    .all()
                )
                ...
                if position == "QB":
                    ...
                    if "pass_attempts" in totals and totals["pass_attempts"] > 0:
                        totals["comp_pct"] = round(
                            totals["completions"] / totals["pass_attempts"] * 100, 1
                        )
                        totals["yards_per_att"] = round(
                            totals["pass_yards"] / totals["pass_attempts"], 1
                        )
                        totals["pass_td_rate"] = round(
                            totals["pass_td"] / totals["pass_attempts"] * 100, 1
                        )
                        totals["int_rate"] = round(
                            totals["interceptions"] / totals["pass_attempts"] * 100, 1
                        )
                    ...
                    if "rush_attempts" in totals and totals["rush_attempts"] > 0:
                        totals["yards_per_carry"] = round(
                            totals["rush_yards"] / totals["rush_attempts"], 1
                        )
                    if "targets" in totals and totals["targets"] > 0:
                        totals["catch_rate"] = round(
                            totals["receptions"] / totals["targets"] * 100, 1
                        )
                    ...
                    if "targets" in totals and totals["targets"] > 0:
                        ...
                        totals["yards_per_target"] = round(
                            totals["rec_yards"] / totals["targets"], 1
                        )
                    if "receptions" in totals and totals["receptions"] > 0:
                        totals["yards_per_reception"] = round(
                            totals["rec_yards"] / totals["receptions"], 1
                        )
                ...
                if existing_stats and len(existing_stats) > 0:
                    ...
                    half_ppr_stat = next(
                        (s for s in existing_stats if s.stat_type == "half_ppr"), None
                    )
                    if half_ppr_stat:
            ...
            results = {"totals_created": totals_created, "players_processed": len(players)}

            # Log success
            self._log_import(
                "season_totals_calculation",
                "success",
            ...
            self._log_import(
                "season_totals_calculation", "error", f"Error calculating season totals: {str(e)}"
            )
    ...
    async def validate_data(self, season: int) -> Dict[str, Any]:
        ...
        try:
            ...
            player_with_base_stats = (
                self.db.query(Player)
                .join(BaseStat, Player.player_id == BaseStat.player_id)
                .filter(BaseStat.season == season)
                .distinct()
                .count()
            )
            ...
            for player in players:
                ...
                fantasy_points_stat = next(
                    (s for s in player_base_stats if s.stat_type == "half_ppr"), None
                )
                ...
                if fantasy_points_stat:
                    if (
                        abs(fantasy_points_stat.value - calculated_points) > 0.5
            ...
            # Log success
            self._log_import(
                "data_validation", "success", f"Validated data for season {season}", results
            )
            ...
            self._log_import("data_validation", "error", f"Error validating data: {str(e)}")
    ...
    def _get_player_base_stats(self, player_id: str, season: int) -> List[BaseStat]:
        ...
        return (
            ...
            .filter(BaseStat.player_id == player_id, BaseStat.season == season)
            .all()
        )
    ...
    def _calculate_fantasy_points(self, stats: Dict[str, Any], position: str) -> float:
        ...
        # Passing points - use safe_float to handle potentially null values
        points += safe_float(safe_dict_get(stats, "pass_yards"), 0.0) * 0.04  # 1 point per 25 yards
        points += safe_float(safe_dict_get(stats, "pass_td"), 0.0) * 4  # 4 points per passing TD
        points -= safe_float(safe_dict_get(stats, "interceptions"), 0.0) * 1  # -1 point per interception

        # Rushing points
        points += safe_float(safe_dict_get(stats, "rush_yards"), 0.0) * 0.1  # 1 point per 10 yards
        points += safe_float(safe_dict_get(stats, "rush_td"), 0.0) * 6  # 6 points per rushing TD

        # Receiving points
        points += safe_float(safe_dict_get(stats, "rec_yards"), 0.0) * 0.1  # 1 point per 10 yards
        points += safe_float(safe_dict_get(stats, "rec_td"), 0.0) * 6  # 6 points per receiving TD
        points += safe_float(safe_dict_get(stats, "receptions"), 0.0) * 0.5  # 0.5 points per reception (half-PPR)
    ...
    def _log_import(self, operation: str, status: str, message: str, details: Optional[Dict[str, Any]] = None) -> None:
        """
        ...
        try:
            ...
            # Create safe dictionary of log details
