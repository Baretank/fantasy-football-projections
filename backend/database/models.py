...
class GameStats(Base):
    ...
    game_stat_id: Mapped[str] = mapped_column(
    ...
    def from_game_log(cls, player_id: str, game_log_row: Dict) -> "GameStats":
        ...
        stats_fields = {
            k: v
            for k, v in game_log_row.items()
            if k not in ["date", "week", "opp", "game_location", "result", "team_pts", "opp_pts"]
        }

        return cls(game_stat_id=str(uuid.uuid4()), **base_data, stats=stats_fields)
...
class Player(Base):
    ...
    )  # Active, Injured, Rookie
    depth_chart_position: Mapped[str] = mapped_column(
        String, default="Backup"
    )  # Starter, Backup, Reserve
    is_fill_player: Mapped[bool] = mapped_column(
        Boolean, default=False
    )  # Whether this is a fill player (for team stats reconciliation)
    is_rookie: Mapped[bool] = mapped_column(
        Boolean, default=False, index=True
    )  # Whether this is a rookie player

    # Draft information fields
    draft_position: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True
    )  # Overall draft position
    draft_team: Mapped[Optional[str]] = mapped_column(
    ...
    draft_status: Mapped[str] = mapped_column(
        Enum(DraftStatus), default=DraftStatus.AVAILABLE, index=True
    )  # available, drafted, watched
    fantasy_team: Mapped[Optional[str]] = mapped_column(
        String, nullable=True, index=True
    )  # Fantasy team that drafted the player
    draft_order: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True
    )  # Order in which player was drafted
...
class Projection(Base):
    ...
    scenario_id: Mapped[Optional[str]] = mapped_column(
        ForeignKey("scenarios.scenario_id"), nullable=True, index=True
    )
    season: Mapped[int] = mapped_column(Integer, nullable=False, index=True)
...
class StatOverride(Base):
    ...
    player_id: Mapped[str] = mapped_column(ForeignKey("players.player_id"), nullable=False)
    projection_id: Mapped[str] = mapped_column(
        ForeignKey("projections.projection_id"), nullable=False
    )
...
class DraftBoard(Base):
    ...
    draft_board_id: Mapped[str] = mapped_column(
        String, primary_key=True, default=lambda: str(uuid.uuid4())
    )
    name: Mapped[str] = mapped_column(String, nullable=False)
    ...
    settings: Mapped[Optional[Dict]] = mapped_column(
        JSON, nullable=True
    )  # scoring rules, position limits, etc.
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(
        DateTime, default=datetime.utcnow, onupdate=datetime.utcnow
    )
