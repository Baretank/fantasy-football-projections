...
async def import_position_weekly_stats(
    service: NFLDataImportService, season: int, player_ids: List[str]
) -> dict:
...
async def calculate_position_totals(
    service: NFLDataImportService, season: int, position: str
) -> dict:
    ...
    try:
        ...
        players = (
            ...
            .filter(GameStats.season == season, Player.position == position)
            .distinct()
            .all()
        )
...
async def validate_position_data(service: NFLDataImportService, season: int, position: str) -> dict:
    ...
    try:
        ...
        players = (
            ...
            .filter(BaseStat.season == season, Player.position == position)
            .distinct()
            .all()
        )
...
async def main():
    ...
    parser.add_argument(
        "--position",
        choices=["QB", "RB", "WR", "TE", "team", "all"],
        required=True,
        help='Position to import or "team" for team stats or "all" for all positions',
    )
