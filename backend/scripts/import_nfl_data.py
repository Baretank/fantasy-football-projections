...
async def import_specific_data(
    seasons: List[int],
    data_type: str,
        ...
        for season in seasons:
            ...
            try:
                if data_type == "players":
                    ...
                    if (
                        hasattr(service.import_players, "__code__")
                        and "limit" in service.import_players.__code__.co_varnames
                    ):
                        ...
                        logger.warning(
                            "The service doesn't support the limit parameter. Importing all data."
                        )
                    ...
                    if player_limit:
                        ...
                        results = await service.import_weekly_stats(
                            season, player_limit=player_limit
                        )
                    else:
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Import NFL data using the NFL data import service"
    )
    parser.add_argument(
        "--seasons", type=int, nargs="+", required=True, help="Seasons to import (e.g., 2022 2023)"
    )
    parser.add_argument(
        "--type",
        choices=["full", "players", "weekly", "team", "totals", "validate"],
        default="full",
    ...
    parser.add_argument(
        "--limit",
        type=int,
        help="Limit number of records for testing (only applicable for players)",
    )
    parser.add_argument(
        "--player_limit", type=int, help="Limit number of players for weekly stats import"
    )
