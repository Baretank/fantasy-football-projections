...
def get_db_stats(db: Any) -> Dict[str, Any]:
...
def get_import_stats(db: Any) -> Dict[str, Any]:
...
def check_data_consistency(db: Any, season: Optional[int] = None) -> Dict[str, Any]:
    """Check data consistency between game stats and base stats."""
    results: Dict[str, Any] = {"total_checked": 0, "mismatches_found": 0, "positions_checked": {}}
    ...
    for player in players:
        ...
        if player.position not in results["positions_checked"]:
            results["positions_checked"][player.position] = {"total": 0, "mismatches": 0}

        # Get all seasons for this player
        seasons = (
            db.query(BaseStat.season)
            .filter(BaseStat.player_id == player.player_id)
            .distinct()
            .all()
        )
        ...
        for (player_season,) in seasons:
            ...
            base_stats = (
                db.query(BaseStat)
                .filter(BaseStat.player_id == player.player_id, BaseStat.season == player_season)
                .all()
            )
            ...
            game_stats = (
                db.query(GameStats)
                .filter(GameStats.player_id == player.player_id, GameStats.season == player_season)
                .all()
            )
...
def print_consistency_check(results: Dict[str, Any]) -> None:
    ...
    print(
        f"Consistency Rate: {((results['total_checked'] - results['mismatches_found']) / results['total_checked'] * 100):.2f}% OK"
    )
    ...
    for position, stats in results["positions_checked"].items():
        consistency = (
            ((stats["total"] - stats["mismatches"]) / stats["total"] * 100)
            if stats["total"] > 0
            else 0
        )
        position_data.append([position, stats["total"], stats["mismatches"], f"{consistency:.2f}%"])
...
def main() -> None:
    ...
    parser.add_argument(
        "--check-consistency", action="store_true", help="Run data consistency check"
    )
    ...
    try:
        # We already imported it at the top, just make sure it's available
        pass
    except ImportError:
