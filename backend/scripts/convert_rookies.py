...
def convert_to_json(input_file: str, output_file: str, force: bool = False):
    ...
    for _, row in df.iterrows():
        rookie = {
            ...
            "draft_position": (
                int(float(row["ADP"])) if "ADP" in df.columns and pd.notna(row["ADP"]) else 0
            ),
            "projected_stats": {
        ...
        if height_field and pd.notna(row[height_field]):
            ...
            # Check if height is in ft-in format (e.g., "6-2")
                ...
                try:
                    ...
                    logger.warning(
                        f"Invalid height format for {rookie['name']}: {row[height_field]}"
                    )
...
def main():
    ...
    parser.add_argument(
        "-f", "--force", action="store_true", help="Force overwrite of existing output file"
    )
