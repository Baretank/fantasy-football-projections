...
def check_players(
    db, position: Optional[str] = None, team: Optional[str] = None, limit: int = 5
) -> None:
...
def check_player_stats(db, player_name: Optional[str] = None, season: Optional[int] = None) -> None:
    ...
    if player_name:
        ...
        base_stats = (
            db.query(BaseStat).filter(BaseStat.player_id == player.player_id, season_filter).all()
        )
        ...
        for position in ["QB", "RB", "WR", "TE"]:
            ...
            players_with_stats = (
                ...
                .filter(Player.position == position, BaseStat.season == season)
                .distinct()
                .count()
            )
            ...
            top_players = (
                db.query(Player, BaseStat.value)
                .join(BaseStat, Player.player_id == BaseStat.player_id)
                .filter(
    ...
    for season, stats in sorted(stats_by_season.items(), reverse=True):
        ...
        if team:
            ...
            for team_stat in stats[:5]:  # Show first 5
                print(
                    f"- {team_stat.team}: Rank {team_stat.rank}, Pass %: {team_stat.pass_percentage:.1f}%, YPC: {team_stat.rush_yards_per_carry:.2f}"
                )
