        ...
        for _, row in df.iterrows():
            try:
                ...
                if "player_id" in df.columns and pd.notna(row["player_id"]):
                    existing_player = (
                        db.query(Player).filter(Player.player_id == row["player_id"]).first()
                    )
                ...
                if not existing_player:
                    existing_player = (
                        db.query(Player)
                        .filter(Player.name == row["name"], Player.position == row["position"])
                        .first()
                    )
                ...
                if existing_player:
                    ...
                    if "date_of_birth" in df.columns and pd.notna(row["date_of_birth"]):
                        try:
                            existing_player.date_of_birth = pd.to_datetime(
                                row["date_of_birth"]
                            ).date()
                        except:
                            logger.warning(
                                f"Invalid date of birth for {row['name']}: {row['date_of_birth']}"
                            )
                    ...
                    if "depth_chart_position" in df.columns and pd.notna(
                    ...
                    new_player = Player(
                        ...
                        depth_chart_position=(
                            ...
                            and pd.notna(row["depth_chart_position"])
                            else "Backup"
                        ),
                        draft_team=(
                            row["draft_team"]
