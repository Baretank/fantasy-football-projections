    ...
    try:
        ...
        if duplicate_names:
            ...
            for name in duplicate_names:
                ...
                print(
                    f"Keeping {keep.name} (ID: {keep.player_id}, Created: {keep.created_at}, Updated: {keep.updated_at})"
                )

                for player in delete:
                    print(
                        f"Removing {player.name} (ID: {player.player_id}, Created: {player.created_at})"
                    )
                    db.delete(player)
                    total_removed += 1
