...
async def batch_create_projections(
    request: BatchProjectionCreateRequest, db: Session = Depends(get_db)
):
    """
    ...
    result = await service.batch_create_projections(
        player_ids=request.player_ids, season=request.season, scenario_id=request.scenario_id
    )

    if result["success"] == 0 and result["failure"] > 0:
        raise HTTPException(
            status_code=400, detail=f"Failed to create any projections. {result.get('error', '')}"
        )
...
async def batch_adjust_projections(
    request: BatchProjectionAdjustRequest, db: Session = Depends(get_db)
):
    """
    ...
    """
    service = BatchService(db)
    result = await service.batch_adjust_projections(adjustments=request.adjustments)

    if result["success"] == 0 and result["failure"] > 0:
        raise HTTPException(
            status_code=400, detail=f"Failed to adjust any projections. {result.get('error', '')}"
        )
...
async def batch_create_scenarios(
    request: BatchScenarioCreateRequest, db: Session = Depends(get_db)
):
    """
    ...
    """
    service = BatchService(db)
    result = await service.batch_create_scenarios(scenario_templates=request.scenarios)

    if result["success"] == 0 and result["failure"] > 0:
        raise HTTPException(
            status_code=400, detail=f"Failed to create any scenarios. {result.get('error', '')}"
        )
...
async def export_projections(
    ...
    try:
        filename, content = await service.export_projections(
            format=format, filters=request.filters, include_metadata=include_metadata
        )
        ...
        raise HTTPException(status_code=400, detail=f"Failed to export projections: {str(e)}")
...
async def clear_cache(pattern: Optional[str] = None, db: Session = Depends(get_db)):
    """
...
async def import_nfl_data(
    season: int, db: Session = Depends(get_db), background_tasks: BackgroundTasks = None
):
    """
