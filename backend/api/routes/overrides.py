...
async def create_override(request: StatOverrideRequest, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        if not override:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Could not create override"
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error creating override"
        )
...
async def get_player_overrides(player_id: str, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error retrieving overrides"
        )
...
async def get_projection_overrides(projection_id: str, db: Session = Depends(get_db)):
    """
...
@router.delete(
    ...
    responses={
        200: {
            ...
            "content": {
                "application/json": {
                    "example": {"status": "success", "message": "Override deleted successfully"}
                }
            },
        },
...
async def delete_override(override_id: str, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        return SuccessResponse(status="success", message="Override deleted successfully")
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error deleting override"
        )
...
async def batch_override(request: BatchOverrideRequest, db: Session = Depends(get_db)):
    """
