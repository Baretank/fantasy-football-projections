...
async def create_scenario(request: ScenarioRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not scenario:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Could not create scenario"
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error creating scenario"
        )
...
async def get_all_scenarios(db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error retrieving scenarios"
        )
...
async def get_scenario(scenario_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error retrieving scenario"
        )
...
async def get_scenario_projections(
    ...
    try:
        ...
        projections = await scenario_service.get_scenario_projections(
            scenario_id=scenario_id, position=position, team=team
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error retrieving projections"
        )
...
@router.post(
    ...
    responses={
        ...
        400: {"model": ErrorResponse, "description": "Invalid request"},
        404: {"model": ErrorResponse, "description": "Source scenario not found"},
        500: {"model": ErrorResponse, "description": "Internal server error"},
    },
)
async def clone_scenario(
    ...
    try:
        ...
        new_scenario = await scenario_service.clone_scenario(
            source_scenario_id=scenario_id, new_name=name, new_description=description
        )

        if not new_scenario:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Could not clone scenario"
            )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error cloning scenario"
        )
...
@router.delete(
    ...
    responses={
        200: {
            ...
            "content": {
                "application/json": {
                    "example": {"status": "success", "message": "Scenario deleted successfully"}
                }
            },
        },
        404: {"model": ErrorResponse, "description": "Scenario not found"},
...
async def delete_scenario(scenario_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return SuccessResponse(status="success", message="Scenario deleted successfully")
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error deleting scenario"
        )
...
async def compare_scenarios(request: ScenarioComparisonRequest, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        comparison = await scenario_service.compare_scenarios(
            scenario_ids=request.scenario_ids, position=request.position
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error comparing scenarios"
        )
