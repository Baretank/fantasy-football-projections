...
async def get_projection(projection_id: str, db: Session = Depends(get_db)):
...
async def get_projections(
    ...
    projections = await service.get_player_projections(
        player_id=player_id, team=team, season=season, scenario_id=scenario_id
    )
...
async def create_projection(request: ProjectionCreateRequest, db: Session = Depends(get_db)):
    """Create a new baseline projection."""
    service = ProjectionService(db)
    projection = await service.create_base_projection(
        player_id=request.player_id, season=request.season, scenario_id=request.scenario_id
    )

    if not projection:
        raise HTTPException(status_code=400, detail="Failed to create projection")

    return projection


@router.post(
    "/{projection_id}/adjust", response_model=ProjectionResponse
)  # Changed to POST for test compatibility
async def adjust_projection(
    projection_id: str, adjustments: ProjectionAdjustRequest, db: Session = Depends(get_db)
):
    """Update a projection with adjustments."""
    service = ProjectionService(db)
    projection = await service.update_projection(
        projection_id=projection_id, adjustments=adjustments.adjustments
    )

    if not projection:
        raise HTTPException(status_code=400, detail="Failed to adjust projection")
...
async def get_projection_range(
    ...
    proj_range = await service.generate_projection_range(
        projection_id=projection_id, confidence=confidence, scenarios=create_scenarios
    )

    if not proj_range:
        raise HTTPException(status_code=400, detail="Failed to generate projection range")
...
async def get_projection_variance(
    projection_id: str, use_historical: bool = True, db: Session = Depends(get_db)
):
    """
    ...
    variance_data = await service.calculate_variance(
        projection_id=projection_id, use_historical=use_historical
    )

    if not variance_data:
        raise HTTPException(status_code=400, detail="Failed to calculate variance")
...
async def create_rookie_projections(
    ...
    success_count, errors = await service.create_rookie_projections(
        season=season, scenario_id=scenario_id
    )
...
async def enhance_rookie_projection(
    ...
    projection = await service.enhance_rookie_projection(
        player_id=player_id, comp_level=comp_level, playing_time_pct=playing_time_pct, season=season
    )

    if not projection:
        raise HTTPException(status_code=400, detail="Failed to enhance rookie projection")
...
async def adjust_team_projections(
    ...
    try:
        ...
        if not adjustments and not player_shares:
            raise HTTPException(
                status_code=400, detail="Must provide either adjustments or player_shares"
            )

        logger.info(
            f"Adjusting team projections for {team}, season {season}, scenario_id {scenario_id}"
        )
        ...
        if scenario_id:
            ...
            projections_exist = (
                db.query(Projection)
                .filter(and_(Projection.scenario_id == scenario_id, Projection.season == season))
                .count()
                > 0
            )
        ...
        if not updated_projections:
            raise HTTPException(status_code=400, detail="Failed to apply team adjustments")

        return updated_projections
    except Exception as e:
        logger.error(f"Error in adjust_team_projections: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error applying team adjustments: {str(e)}")


@router.get("/team/{team}/usage", response_model=Dict)
async def get_team_usage_breakdown(
    team: str, season: int = Query(..., ge=2023), db: Session = Depends(get_db)
):
    """
    ...
    """
    service = TeamStatService(db)
    usage_data = await service.get_team_usage_breakdown(team=team, season=season)

    if not usage_data:
        raise HTTPException(status_code=400, detail="Failed to retrieve team usage breakdown")
...
async def get_team_stats(
    ...
    """
    service = TeamStatService(db)
    stats = await service.get_team_stats(team=team, season=season)
...
async def create_draft_based_rookie_projection(
    ...
    projection = await service.create_draft_based_projection(
        player_id=player_id, draft_position=draft_position, season=season, scenario_id=scenario_id
    )
