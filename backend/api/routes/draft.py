...
async def get_draft_board(
    ...
    result = await draft_service.get_draft_board(
        status=status, position=position, team=team, order_by=order_by, limit=limit, offset=offset
    )
    return result


@router.post("/draft-status")
async def update_draft_status(update: DraftStatusUpdate, db: Session = Depends(get_db)):
    """
...
async def batch_update_draft_status(updates: BatchDraftStatusUpdate, db: Session = Depends(get_db)):
    """
...
async def reset_draft(db: Session = Depends(get_db)):
    """
...
async def undo_last_draft_pick(db: Session = Depends(get_db)):
    """
...
async def get_draft_progress(db: Session = Depends(get_db)):
    """
...
async def create_draft_board(board: DraftBoardCreate, db: Session = Depends(get_db)):
    """
    ...
    result = await draft_service.create_draft_board(
        name=board.name, description=board.description, season=board.season, settings=board.settings
    )
...
async def get_draft_boards(active_only: bool = Query(True), db: Session = Depends(get_db)):
    """
