...
class TestDataImportTransformations:
    ...
    def sample_player(self, test_db):
        ...
        player = Player(player_id=str(uuid.uuid4()), name="Test Player", team="KC", position="QB")
        test_db.add(player)
        test_db.commit()
        return player
    ...
    async def test_calculate_season_totals(self, service, sample_player, sample_game_stats):
        ...
        base_stats = (
            ...
            .filter(BaseStat.player_id == sample_player.player_id, BaseStat.season == 2023)
            .all()
        )
    ...
    async def test_stat_consistency_across_positions(self, service, test_db, sample_player):
        ...
        for position in positions:
            ...
            service.db.query(Player).filter(Player.player_id == sample_player.player_id).update(
                {"position": position}
            )
            service.db.commit()
