...
class TestTeamStatService:
    ...
    async def test_validate_team_stats(self, team_stats_service, team_stats_2024):
        ...
        print(
            f"- Plays match: {abs(kc_stats.pass_attempts + kc_stats.rush_attempts - kc_stats.plays) < 0.01}"
        )
    ...
    @pytest.mark.asyncio
    async def test_apply_team_adjustments(
        self, team_stats_service, sample_team_projections, team_stats_2024, sample_players, test_db
    ):
        ...
        adjusted_projs = await team_stats_service.apply_team_stats_directly(
            original_stats=orig_kc_stats, new_stats=new_kc_stats, players=sample_team_projections
        )
        ...
        qb_proj = next(
            p for p in adjusted_projs if p.player_id == sample_players["ids"]["Patrick Mahomes"]
        )
        ...
        te_proj = next(
            p for p in adjusted_projs if p.player_id == sample_players["ids"]["Travis Kelce"]
        )
    ...
    async def test_calculate_team_adjustment_factors(self, team_stats_service, team_stats_2024):
        ...
        factors = team_stats_service.calculate_team_adjustment_factors(
            original_stats=orig_kc_stats, new_stats=new_kc_stats
        )
    ...
    async def test_get_team_adjustment_factors(self, team_stats_service, team_stats_2024, test_db):
        ...
        assert factors["pass_efficiency"] == pytest.approx(
            pass_yd_per_att_2024 / pass_yd_per_att_2023, rel=0.01
        )
