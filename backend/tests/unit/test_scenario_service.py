...
class TestScenarioService:
    ...
    def baseline_scenario(self, service):
        ...
        async def _create_baseline():
            scenario = await service.create_scenario(
                name="Baseline 2024", description="Standard baseline projections", is_baseline=True
            )
            return scenario
    ...
    async def test_create_scenario(self, service):
        ...
        scenario = await service.create_scenario(
            name="Test Scenario", description="A test scenario", is_baseline=False
        )
    ...
    async def test_get_scenario_projections(self, service, sample_projections):
        ...
        # Filter by position
        qb_projections = await service.get_scenario_projections(scenario.scenario_id, position="QB")
        ...
        kc_qb_projections = await service.get_scenario_projections(
            scenario.scenario_id, position="QB", team="KC"
        )
    ...
    async def test_delete_scenario(self, service, scenario_with_overrides):
        ...
        remaining_projections = (
            ...
            .filter(Projection.scenario_id == scenario.scenario_id)
            .count()
        )
    ...
    async def test_get_all_scenarios(self, service, baseline_scenario):
        ...
        alt_scenario = await service.create_scenario(
            name="Alternative Scenario", description="Another test scenario", is_baseline=False
        )
    ...
    async def test_generate_fill_players(self, service, setup_for_fill_players, team_stats_2024):
        ...
        fill_projections = await service.generate_fill_players(
            scenario_id=scenario.scenario_id, team="KC", season=2024
        )
        ...
        for proj in fill_projections:
            ...
            fill_player = (
                service.db.query(Player).filter(Player.player_id == proj.player_id).first()
            )
        ...
        assert (
            abs(team_totals["rush_attempts"] - team_stats_2024.rush_attempts) < 0.1
        )  # Using standardized field name
