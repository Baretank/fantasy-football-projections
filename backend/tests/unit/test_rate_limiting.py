...
class TestRateLimiting:
    ...
    async def test_backoff_on_rate_limit(self, adapter):
        ...
        async def mock_get(*args, **kwargs):
            ...
            if get_call_count == 1:
                raise aiohttp.ClientResponseError(request_info=MagicMock(), history=(), status=429)
    ...
    async def test_circuit_breaker_pattern(self, adapter, capsys):
        ...
        try:
            ...
            async def mock_get(*args, **kwargs):
                ...
                print(
                    f"  → Failures now: {adapter.circuit_breaker_failures}/{adapter.circuit_breaker_threshold}"
                )
            ...
            circuit_breaker_exceptions = [
                r
                for r in results
                if isinstance(r, Exception) and "circuit breaker" in str(r).lower()
            ]
            ...
            assert (
                len(circuit_breaker_exceptions) > 0
            ), "Expected at least one circuit breaker exception"
            ...
            assert (
                len(circuit_breaker_exceptions) > 0
            ), "Circuit breaker exceptions should be present"
        finally:
    ...
    async def test_request_throttling(self, adapter):
        ...
        with (
            ...
            patch.object(adapter, "throttle_delay", 0.2),
        ):  # 200ms throttle
            ...
            assert (
                len(sleep_delays) >= 1
            ), "Sleep should have been called at least once for throttling"
            ...
            assert (
                sleep_delays[0] <= 0.2
            ), f"Sleep delay should be <= throttle_delay, got {sleep_delays[0]}"
