...
from backend.api.schemas import ProjectionResponse, RookieProjectionResponse, TeamStatsResponse
...
class TestProjectionsRoutes:
    ...
    def test_get_projections_with_filters(self):
        ...
        with patch("backend.api.routes.projections.ProjectionService") as mock_service:
            ...
            service_instance.get_player_projections.assert_called_once_with(
                player_id=player_id, team=None, season=season, scenario_id=None
            )
    ...
    def test_create_projection(self):
        ...
        # Request data
        request_data = {"player_id": player_id, "season": season}
        ...
        with patch("backend.api.routes.projections.ProjectionService") as mock_service:
            ...
            service_instance.create_base_projection.assert_called_once_with(
                player_id=player_id, season=season, scenario_id=None
            )
    ...
    def test_adjust_projection(self):
        ...
        # Request data
        request_data = {"adjustments": {"pass_attempts": 1.1, "rush_volume": 0.9}}
        ...
        with patch("backend.api.routes.projections.ProjectionService") as mock_service:
            ...
            service_instance.update_projection.assert_called_once_with(
                projection_id=projection_id, adjustments=request_data["adjustments"]
            )
    ...
    def test_get_projection_range(self):
        ...
        # Mock response from service
        mock_range = {
            "base": {"half_ppr": 275.5, "pass_yards": 4500.0, "pass_td": 32.0},
            "low": {"half_ppr": 240.2, "pass_yards": 4100.0, "pass_td": 28.0},
            "median": {"half_ppr": 275.5, "pass_yards": 4500.0, "pass_td": 32.0},
            "high": {"half_ppr": 310.8, "pass_yards": 4900.0, "pass_td": 36.0},
            "scenario_ids": None,
        }
        ...
        with patch("backend.api.routes.projections.ProjectionVarianceService") as mock_service:
            ...
            service_instance.generate_projection_range.assert_called_once_with(
                projection_id=projection_id, confidence=0.85, scenarios=False
            )
    ...
    def test_get_team_stats(self):
        ...
        with patch("backend.api.routes.projections.TeamStatService") as mock_service:
            ...
            # Verify service was called correctly
            service_instance.get_team_stats.assert_called_once_with(team=team, season=season)
    ...
    def test_adjust_team_projections(self):
        ...
        # Request data
        adjustments = {"pass_volume": 1.1, "rush_volume": 0.95}
        ...
        with patch("backend.api.routes.projections.TeamStatService") as mock_service:
            service_instance = mock_service.return_value
            service_instance.apply_team_adjustments = AsyncMock(
                return_value=mock_updated_projections
            )

            # Make request with adjustments in the body
            response = client.put(
                f"/projections/team/{team}/adjust?season={season}",
    ...
    def test_create_draft_based_rookie_projection(self):
        ...
        with patch("backend.api.routes.projections.RookieProjectionService") as mock_service:
            ...
            response = client.post(
                f"/projections/rookies/draft-based?player_id={player_id}&draft_position={draft_position}&season={season}"
            )

            # Verify service was called correctly
            service_instance.create_draft_based_projection.assert_called_once_with(
                player_id=player_id, draft_position=draft_position, season=season, scenario_id=None
            )
