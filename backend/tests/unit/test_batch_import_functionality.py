...
class TestBatchImportFunctionality:
    ...
    async def test_batch_import_position(
        self, mock_import, data_service, batch_service, sample_players
    ):
    ...
    @pytest.mark.asyncio
    @patch.object(NFLDataImportService, "import_player_data")
    async def test_batch_size_and_delay(
        self, mock_import, data_service, batch_service, sample_players
    ):
        ...
        if len(timestamps) >= batch_size + 1:
            assert (
                timestamps[batch_size] - timestamps[batch_size - 1] >= delay * 0.9
            )  # Allow for slight timing variance

    @pytest.mark.asyncio
    @patch.object(NFLDataImportService, "import_player_data")
    async def test_partial_batch_failures(
        self, mock_import, data_service, batch_service, sample_players
    ):
