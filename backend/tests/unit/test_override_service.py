...
class TestOverrideService:
    ...
    async def test_create_override(self, service, sample_projection):
        ...
        updated_proj = (
            ...
            .filter(Projection.projection_id == sample_projection.projection_id)
            .first()
        )
        ...
        assert (
            abs(updated_proj.completions - expected_completions) < 1.0
    ...
    async def test_dependent_stat_recalculation_qb(self, service, test_db, sample_players):
        ...
        updated_proj = (
            service.db.query(Projection)
            .filter(Projection.projection_id == test_projection.projection_id)
            .first()
        )
    ...
    async def test_dependent_stat_recalculation_rb(self, service, sample_rb_projection):
        ...
        updated_proj = (
            service.db.query(Projection)
            .filter(Projection.projection_id == sample_rb_projection.projection_id)
            .first()
        )
    ...
    @pytest.mark.asyncio
    async def test_batch_override(
        self, service, sample_projection, sample_rb_projection, sample_wr_projection
    ):
        ...
        wr_proj = (
            service.db.query(Projection)
            .filter(Projection.projection_id == sample_wr_projection.projection_id)
            .first()
        )
    ...
    async def test_apply_overrides_to_projection(self, service, test_db, sample_players):
        ...
        db_proj = (
            service.db.query(Projection).filter(Projection.projection_id == projection_id).first()
        )
    ...
    async def test_override_cascading_effects(self, service, test_db, sample_players):
        ...
        updated_proj = (
            service.db.query(Projection)
            .filter(Projection.projection_id == projection.projection_id)
            .first()
        )
        ...
        assert (
            abs(updated_proj.pass_yards - expected_yards) < 1.0
        )  # Allow small rounding differences
        ...
        assert (
            abs(updated_proj.interceptions - expected_ints) < 0.5
        )  # Allow small rounding differences
    ...
    async def test_multiple_conflicting_overrides(self, service, sample_wr_projection):
        ...
        assert (
            abs(proj_after_first.receptions - expected_receptions) < 1.0
        )  # Allow small rounding differences

        # The ratio stats should remain the same
        assert (
            abs(proj_after_first.catch_pct - original_catch_pct) < 0.01
        )  # Allow small rounding differences
        assert (
            abs(proj_after_first.yards_per_target - original_yards_per_target) < 0.01
        )  # Allow small rounding differences
        ...
        assert round(proj_after_second.yards_per_target, 3) == round(
            proj_after_first.yards_per_target, 3
        )
