...
class TestBatchRoutes:
    ...
    def test_batch_adjust_projections(self):
        ...
        mock_result = {
            ...
            "failure": 0,
            "projection_ids": {
                projection_id_1: projection_id_1,
                projection_id_2: projection_id_2
            }
        }
    ...
    def test_export_projections_csv(self):
        ...
        # Mock CSV content
        mock_csv = "player_id,name,team,position,half_ppr\n" + \
                   f"{str(uuid.uuid4())},Patrick Mahomes,KC,QB,350.5\n"
        
        # Patch the service method
        with patch('backend.api.routes.batch.BatchService') as mock_service:
            service_instance = mock_service.return_value
            service_instance.export_projections = AsyncMock(return_value=("projections.csv", mock_csv))
    ...
    def test_export_projections_json(self):
        ...
        with patch('backend.api.routes.batch.BatchService') as mock_service:
            service_instance = mock_service.return_value
            service_instance.export_projections = AsyncMock(return_value=("projections.json", mock_json))
            ...
            # Verify response
            assert response.status_code == 200
            assert response.headers["Content-Disposition"] == "attachment; filename=projections.json"
