...
class TestTeamAdjustmentIntegration:
    ...
    async def test_team_adjustment_factors(self, team_stat_service, setup_team_data):
        ...
        factors = await team_stat_service.get_team_adjustment_factors(
            team=team, from_season=season, to_season=projection_season
        )
        ...
        expected_pass_volume = (
            setup_team_data["team_stat_2024"].pass_attempts
            / setup_team_data["team_stat_2023"].pass_attempts
        )
        expected_rush_volume = (
            setup_team_data["team_stat_2024"].rush_attempts
            / setup_team_data["team_stat_2023"].rush_attempts
        )
    ...
    @pytest.mark.asyncio
    async def test_apply_team_adjustments_with_projections(
        self, team_stat_service, setup_team_data, test_db
    ):
        ...
        for proj in projections:
            ...
            if hasattr(proj, "pass_yards") and proj.pass_yards:
                ...
                hasattr(proj, "rush_yards")
                and proj.rush_yards
                and hasattr(proj, "rush_attempts")
                and proj.rush_attempts > 200
            ):
                proj.calculate_fantasy_points = lambda: 200
            elif (
                hasattr(proj, "rec_yards")
                and proj.rec_yards
                and hasattr(proj, "rec_td")
                and proj.rec_td > 7
            ):
        ...
        updated_projections = await team_stat_service.apply_team_stats_directly(
            original_stats=original_team_stats, new_stats=new_team_stats, players=projections
        )
        ...
        if qb_player:
            qb_proj = next(
                (p for p in updated_projections if p.player_id == qb_player.player_id), None
            )
            if qb_proj and hasattr(qb_proj, "pass_attempts"):
                pass_volume_factor = (
                    new_team_stats.pass_attempts / original_team_stats.pass_attempts
                )
                assert qb_proj.pass_attempts > 580
                assert abs(qb_proj.pass_attempts / 580 - pass_volume_factor) < 0.1

    @pytest.mark.asyncio
    async def test_apply_team_adjustments_by_team(
        self, team_stat_service, setup_team_data, test_db
    ):
        ...
        factors = team_stat_service.calculate_team_adjustment_factors(
            original_stats=original_team_stats, new_stats=new_team_stats
        )
