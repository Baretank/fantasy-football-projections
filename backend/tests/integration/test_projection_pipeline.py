...
class TestProjectionPipeline:
    ...
    def setup_test_data(self, test_db):
        ...
        for team in teams:
            ...
            players.extend(
                [
                    ...
                    Player(
                        player_id=str(uuid.uuid4()), name=f"{team} RB2", team=team, position="RB"
                    ),
                    Player(
                        player_id=str(uuid.uuid4()), name=f"{team} WR1", team=team, position="WR"
                    ),
                    Player(
                        player_id=str(uuid.uuid4()), name=f"{team} WR2", team=team, position="WR"
                    ),
                    Player(
        ...
        for player in players:
            ...
            if player.position == "QB":
                stats = [
                    BaseStat(
                        player_id=player.player_id, season=season, stat_type="games", value=17.0
                    ),
                    BaseStat(
                    ...
                    BaseStat(
                        player_id=player.player_id, season=season, stat_type="pass_td", value=28.0
                    ),
                    BaseStat(
                        player_id=player.player_id,
                        season=season,
                    ...
                    BaseStat(
                        player_id=player.player_id, season=season, stat_type="rush_td", value=2.0
                    ),
                ]
                ...
                if "RB1" in player.name:
                    ...
                    stats = [
                        BaseStat(
                            player_id=player.player_id, season=season, stat_type="games", value=16.0
                        ),
                        BaseStat(
                            player_id=player.player_id,
                            season=season,
                        ...
                        BaseStat(
                            player_id=player.player_id, season=season, stat_type="rec_td", value=2.0
                        ),
                    ]
                else:
                    # Backup
                    stats = [
                        ...
                        BaseStat(
                            player_id=player.player_id, season=season, stat_type="rec_td", value=1.0
                        ),
                    ]
                ...
                if "WR1" in player.name:
                    ...
                    stats = [
                        ...
                        BaseStat(
                            player_id=player.player_id, season=season, stat_type="rec_td", value=7.0
                        ),
                        BaseStat(
                            player_id=player.player_id,
                            season=season,
                ...
                stats = [
                    ...
                    BaseStat(
                        player_id=player.player_id, season=season, stat_type="targets", value=80.0
                    ),
                    BaseStat(
                        player_id=player.player_id,
                        season=season,
                    ...
                    BaseStat(
                        player_id=player.player_id, season=season, stat_type="rec_td", value=5.0
                    ),
                ]
    ...
    async def test_create_base_projection(self, projection_service, setup_test_data):
        ...
        projection = await projection_service.create_base_projection(
            player.player_id, projection_season
        )
    ...
    async def test_apply_projection_adjustments(self, projection_service, setup_test_data):
        ...
        # Create base projection
        base_projection = await projection_service.create_base_projection(
            player.player_id, projection_season
        )
        ...
        updated_projection = await projection_service.update_projection(
            projection_id=base_projection.projection_id, adjustments=adjustments
        )
    ...
    @pytest.mark.asyncio
    async def test_team_level_adjustments(
        self, team_stat_service, projection_service, setup_test_data
    ):
        ...
        projections_before = await projection_service.get_player_projections(
            team=team, season=projection_season
        )
        ...
        for proj in projections_before:
            original_values[proj.player_id] = {
                ...
                or 0.01,  # Avoid division by zero
        ...
        updated_projections = await team_stat_service.apply_team_adjustments(
            team=team, season=projection_season, adjustments=team_adjustments
        )
    ...
    @pytest.mark.asyncio
    async def test_player_share_adjustments(
        self, team_stat_service, projection_service, setup_test_data
    ):
        ...
        wr1_proj_before = await projection_service.get_projection_by_player(
            wr1.player_id, projection_season
        )
        ...
        target_share_adjustment = {"target_share": 0.3}  # Set to 30% target share

        # Update the projection directly
        updated_proj = await projection_service.update_projection(
            projection_id=wr1_proj_before.projection_id, adjustments=target_share_adjustment
        )
    ...
    async def test_complete_projection_pipeline(
        self, projection_service, team_stat_service, validation_service, setup_test_data
    ):
        ...
        base_projections = []
        for player in team_players:
            proj = await projection_service.create_base_projection(
                player.player_id, projection_season
            )
        ...
        updated_wr1_proj = await projection_service.update_projection(
            projection_id=wr1_proj.projection_id, adjustments=wr1_adjustments
        )
    ...
    async def import_test_data(self, test_db):
        ...
        player = Player(
            player_id=str(uuid.uuid4()), name="Test Import Player", team="SF", position="RB"
        )
        test_db.add(player)
        ...
        with (
            patch.object(service, "_fetch_game_log_data", return_value=mock_game_log),
    ...
    async def test_import_to_projection_pipeline(
        self, import_test_data, projection_service, validation_service, team_stats_2024
    ):
        ...
        base_stats = (
            data_import_service.db.query(BaseStat)
            .filter(BaseStat.player_id == player.player_id, BaseStat.season == 2023)
            .all()
        )
        ...
        if "rush_attempts" in stats_dict and stats_dict["rush_attempts"] > 0:
            ...
            assert (
                0.7 * stats_dict["rush_attempts"]
                <= projection.rush_attempts
                <= 1.3 * stats_dict["rush_attempts"]
            )
        ...
        updated_projection = await projection_service.update_projection(
            projection_id=projection.projection_id, adjustments=adjustments
        )
    ...
    @pytest.mark.asyncio
    async def test_batch_import_and_projection(self, setup_test_data, projection_service, test_db):
        ...
        with patch.object(batch_service, "process_batch", return_value=successful_imports):
            ...
            common_adjustments = {"td_rate": 1.05}  # 5% more TDs for everyone

            updated_projections = []
            for proj in projections:
                updated_proj = await projection_service.update_projection(
                    projection_id=proj.projection_id, adjustments=common_adjustments
                )
            ...
            for i, proj in enumerate(updated_projections):
                ...
                fresh_data = (
                    ...
                    .filter(Projection.projection_id == proj.projection_id)
                    .first()
                )
    ...
    async def test_override_and_projection_integration(
        self, setup_test_data, projection_service, override_service
    ):
        ...
        original_yards_per_att = (
            original_pass_yards / original_pass_attempts if original_pass_attempts > 0 else 0
        )
        new_yards_per_att = (
            updated_qb_proj.pass_yards / new_pass_attempts if new_pass_attempts > 0 else 0
        )
