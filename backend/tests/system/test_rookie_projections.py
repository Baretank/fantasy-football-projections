...
class TestRookieProjections:
    ...
    def setup_rookie_data(self, test_db):
        ...
        return {"teams": teams, "rookies": rookies, "current_season": current_season}
    ...
    async def test_top_qb_rookie_projection(self, rookie_service, setup_rookie_data, test_db):
        ...
        # Get a top QB rookie
        qb_rookie = next(
            r for r in setup_rookie_data["rookies"] if r.position == "QB" and r.draft_pick == 1
        )
    ...
    async def test_mid_round_qb_rookie_projection(self, rookie_service, setup_rookie_data, test_db):
        ...
        qb_rookie = next(
            r for r in setup_rookie_data["rookies"] if r.position == "QB" and r.draft_pick == 15
        )
    ...
    async def test_top_rb_rookie_projection(self, rookie_service, setup_rookie_data, test_db):
        ...
        # Get a top RB rookie
        rb_rookie = next(
            r for r in setup_rookie_data["rookies"] if r.position == "RB" and r.draft_pick == 5
        )
    ...
    async def test_all_rookie_projections(self, rookie_service, setup_rookie_data, test_db):
        ...
        if len(qb_projections) >= 2:
            qb_projs_sorted = sorted(
                qb_projections,
                key=lambda p: test_db.query(Player)
