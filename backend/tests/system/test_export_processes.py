...
class TestExportProcesses:
    ...
    def sample_data(self, test_db):
        ...
        test_db.commit()
        logger.debug(f"Created sample data with {len(players)} players and {len(projections)} projections")
    ...
    def test_export_csv_format(self, test_client, sample_data, test_db):
        ...
        assert abs(float(mahomes_row["half_ppr"]) - 350.5) < 0.1, f"Wrong fantasy points: {mahomes_row['half_ppr']}"
        assert int(mahomes_row["pass_attempts"]) == 580, f"Wrong pass attempts: {mahomes_row['pass_attempts']}"
