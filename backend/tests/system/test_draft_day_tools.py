...
class TestDraftDayTools:
    ...
    def test_auto_projection_after_draft(self, test_client, sample_players, test_db):
        ...
        if response.status_code != 200:
            logger.debug(f"Draft with projection response: {response.status_code} - {response.text}")
    ...
    def test_get_draft_board_filtered(self, test_client, sample_players, test_db):
        ...
        assert "players" in data, "Response missing 'players' key"
        assert len(data["players"]) == 3, f"Expected 3 available players, got {len(data['players'])}"
    ...
    def test_tracking_draft_progress(self, test_client, sample_players, test_db):
        ...
        assert data["drafted_players"] == 2, f"Expected 2 drafted players, got {data['drafted_players']}"
        assert data["available_players"] == 2, f"Expected 2 available players, got {data['available_players']}"
    ...
    def test_undo_last_draft_pick(self, test_client, sample_players, test_db):
        ...
        player3 = test_db.query(Player).filter(Player.player_id == "player3").first()
        assert player3.draft_status == "available", f"Player 3 should be available, got {player3.draft_status}"
        
        # But earlier picks remain drafted
        player1 = test_db.query(Player).filter(Player.player_id == "player1").first()
        player2 = test_db.query(Player).filter(Player.player_id == "player2").first()
        assert player1.draft_status == "drafted", f"Player 1 should be drafted, got {player1.draft_status}"
        assert player2.draft_status == "drafted", f"Player 2 should be drafted, got {player2.draft_status}"