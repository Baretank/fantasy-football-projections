...
class TestPerformance:
    ...
    def large_player_dataset(self, test_db) -> List[Player]:
        ...
        logger.debug(
            f"Setting up large_player_dataset fixture for performance tests (ID: {fixture_id})"
        )
    ...
    def large_projection_dataset(self, test_db, large_player_dataset):
        ...
        fixture_id = str(uuid.uuid4())[:8]
        logger.debug(
            f"Setting up large_projection_dataset fixture for performance tests (ID: {fixture_id})"
        )
        ...
        return {"scenario": base_scenario, "projections": projections, "season": season}
    ...
    def test_projection_list_response_time(self, test_client, large_projection_dataset):
        ...
        response = test_client.get(
            f"/api/projections/projections/?scenario_id={large_projection_dataset['scenario'].scenario_id}"
        )
        end_time = time.time()
    ...
    def test_caching_effectiveness(self, test_client, large_projection_dataset):
        ...
        ), f"Caching should not be slower: {cached_time:.4f} vs {uncached_time:.4f}"
    ...
    def test_filtered_query_performance(self, test_client, large_projection_dataset):
        ...
        for filter_str in test_filters:
            ...
            ), f"Response failed for filter '{filter_str}': {response.text}"
            ...
            assert (
                response_time < 1.0
            ), f"Filtered query too slow: {response_time:.4f} seconds for {filter_str}"
    ...
    def test_memory_usage_with_large_dataset(self, test_client, large_projection_dataset):
        ...
        assert (
            cache_stats["size_mb"] < 50
        ), f"Cache using too much memory: {cache_stats['size_mb']} MB"
    ...
    def test_get_performance_metrics(self, test_client):
        ...
        assert (
            data["response_time_seconds"] < 2.0
        ), f"Metrics endpoint too slow: {data['response_time_seconds']} seconds"

        logger.info(
            f"Performance metrics: Database has {data['database']['total_records']} total records"
        )
        logger.info(
            f"Memory usage: {data['memory']['used_memory_mb']:.1f} MB / {data['memory']['total_memory_mb']:.1f} MB"
        )
    ...
    def test_concurrent_request_performance(self, test_client, large_projection_dataset):
        ...
        for result in all_results:
            ...
            assert (
                result["time"] < 5.0
            ), f"Request too slow: {result['time']:.4f} seconds for {result['url']}"
